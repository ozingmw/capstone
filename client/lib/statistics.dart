...
import 'package:dayclover/service/sentiment_service.dart';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const DesignedTextBox(text: "๑최근 7일 최고감정"),
              const SizedBox(height: 10),
              SizedBox(
                height: 90,
...
class WeeklyTopEmotion extends StatefulWidget {
  ...
  const WeeklyTopEmotion({
    super.key,
    required this.sentimentData,
  });

  @override
  State<WeeklyTopEmotion> createState() => _WeeklyTopEmotionState();
}
...
class _WeeklyTopEmotionState extends State<WeeklyTopEmotion> {
  ...
  List<(String, double)> _getTopEmotions() {
    final Map<String, dynamic> counts =
        widget.sentimentData['res']['sentiment_counts'] as Map<String, dynamic>;
    ...
    int maxCount = 0;
    List<(String, double)> topEmotions = [];
    ...
    counts.forEach((emotion, count) {
      if (count == maxCount) {
        ...
        topEmotions.add((emotion, percentage));
      }
    });
  ...
  void _showOverlay(
      BuildContext context, String emotion, double percentage, GlobalKey key) {
    _removeOverlay();
    ...
    _timer?.cancel();
    _timer = Timer(const Duration(seconds: 2), _removeOverlay);
  }
            ...
            Row(
              ...
              children: [
                Icon(
                  ...
                  color: sentimentColors[topEmotions[0].$1],
                  size: 40,
                ),
                const SizedBox(width: 8),
                Text(
                  '${topEmotions[0].$1}: ${topEmotions[0].$2.toStringAsFixed(1)}%',
                  style: TextStyle(
                    fontSize: 24,
                    color: sentimentColors[topEmotions[0].$1]?.withOpacity(0.7),
                    fontWeight: FontWeight.w500,
                  ),
                ),
              ],
          ...
          ...topEmotions.map((emotion) {
            ...
            return Padding(
              ...
              child: GestureDetector(
                onTap: () => _showOverlay(context, emotion.$1, emotion.$2, key),
                child: Icon(
                  key: key,
                  Icons.filter_vintage,
                  color: sentimentColors[emotion.$1],
                  size: 40,
                ),
              ),
            );
